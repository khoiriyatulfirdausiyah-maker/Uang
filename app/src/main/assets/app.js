...
let persistTimerV812=0;
let persistIdleV812=0;
let persistDirtyV812=false;
let renderFrameV812=0;

function flushStateV812(){
  if(!persistDirtyV812)return;

  if(persistTimerV812){
    clearTimeout(persistTimerV812);
    persistTimerV812=0;
  }

  if(persistIdleV812 && typeof cancelIdleCallback==='function'){
    try{cancelIdleCallback(persistIdleV812)}catch(_){}
  }
  persistIdleV812=0;

  persistDirtyV812=false;
  localStorage.setItem('uangku_data_v3',JSON.stringify(state));
}

function scheduleStatePersistV812(){
  persistDirtyV812=true;
  if(persistTimerV812||persistIdleV812)return;
  ...
  if(typeof requestIdleCallback==='function'){
    persistIdleV812=requestIdleCallback(run,{timeout:240});
  }else{
    persistTimerV812=setTimeout(run,70);
  }
}
...
function save(options={}){
  ...
  scheduleStatePersistV812();
  if(options&&options.immediate)flushStateV812();
}

function scheduleRenderV812(){
  if(renderFrameV812)return;
  const run=()=>{
    renderFrameV812=0;
    render();
  };
  if(typeof requestAnimationFrame==='function'){
    renderFrameV812=requestAnimationFrame(run);
  }else{
    renderFrameV812=setTimeout(run,0);
  }
}

document.addEventListener?.('visibilitychange',()=>{
  if(document.visibilityState==='hidden')flushStateV812();
});
window.addEventListener?.('pagehide',flushStateV812);
window.addEventListener?.('beforeunload',flushStateV812);
...
function navigate(page,opts={}){
 ...
 if(next==='transactions'&&currentPage!=='transactions'){
...
function debtTotals(){
 const x=financeTotalsV812();
 return {payable:x.payable,receivable:x.receivable};
}
...
function assetVisualHTML(tab,a){
 if(a?.logo){
   return `<div class="asset-icon asset-logo-v8131 custom"><img src="${a.logo}" alt="Logo ${esc(a?.name||assetKindLabel(a)||'Aset')}"></div>`;
 }
 return `<div class="asset-icon asset-logo-v8131"><span>${assetIcon(tab,a)}</span></div>`;
}

function assetLogoEditorHTMLV8131(tab,a){
 const has=!!a?.logo;
 const fallback=assetIcon(tab,a||{kind:assetTypeChoices[tab]?.[0]?.[0]||''});
 return `<div class="asset-logo-editor-v8131">
   <div class="asset-logo-preview-v8131 ${has?'custom':''}" id="assetLogoPreviewV8131">
     ${has?`<img src="${a.logo}" alt="Preview logo aset">`:`<span>${fallback}</span>`}
   </div>
   <div class="asset-logo-actions-v8131">
     <input type="file" id="assetLogoFileV8131" accept="image/*" hidden>
     <input type="hidden" id="assetLogoValueV8131" name="logo" value="${esc(a?.logo||'')}">
     <button type="button" class="btn light" id="chooseAssetLogoV8131">${has?'Ganti logo':'Tambah logo'}</button>
     <button type="button" class="btn light ${has?'':'hidden'}" id="removeAssetLogoV8131">Hapus logo</button>
     <small>Tanpa logo custom, ikon bawaan sesuai jenis aset tetap dipakai.</small>
   </div>
 </div>`;
}

function bindAssetLogoEditorV8131(tab,a){
 const input=$('#assetLogoFileV8131'), choose=$('#chooseAssetLogoV8131'),
       remove=$('#removeAssetLogoV8131'), preview=$('#assetLogoPreviewV8131'),
       value=$('#assetLogoValueV8131'), kindSelect=$('#assetKindSelect');

 const fallback=()=>assetIcon(tab,{kind:kindSelect?.value||a?.kind||(assetTypeChoices[tab]?.[0]?.[0]||'')});
 const showFallback=()=>{
   if(!preview)return;
 ...
 input&&(input.onchange=async e=>{
   ...
   try{
     ...
     if(preview){preview.classList.add('custom');preview.innerHTML=`<img src="${data}" alt="Preview logo aset">`;}
     remove?.classList.remove('hidden');
     if(choose)choose.textContent='Ganti logo';
   }catch(err){toast('Logo tidak bisa dibaca.')}
 });
 remove&&(remove.onclick=()=>{
   ...
   if(choose)choose.textContent='Tambah logo';
 });
 if(kindSelect){
   const oldHandler=kindSelect.onchange;
   kindSelect.onchange=e=>{
     oldHandler&&oldHandler.call(kindSelect,e);
     if(!value?.value)showFallback();
   };
 }
}
...
function requestDeleteAccount(id){
 ...
 openConfirm('Hapus Akun',`Hapus akun ${a.name}? Akun dengan saldo Rp 0 bisa dihapus. Riwayat transaksi lama tetap tersimpan.`,`Hapus`,()=>{
   ...
   save();toast('Akun dihapus.');scheduleRenderV812();
 });
}


function budgetSkipKeyV813(month,category){
  return `${String(month||'')}|${String(category||'')}`;
}
function isBudgetSkippedV813(month,category){
  state.budgetSkips=state.budgetSkips&&typeof state.budgetSkips==='object'?state.budgetSkips:{};
  return state.budgetSkips[budgetSkipKeyV813(month,category)]===true;
}
function setBudgetSkipV813(month,category,skip=true){
  state.budgetSkips=state.budgetSkips&&typeof state.budgetSkips==='object'?state.budgetSkips:{};
  const key=budgetSkipKeyV813(month,category);
  if(skip)state.budgetSkips[key]=true;
  else delete state.budgetSkips[key];
}
function budgetsForMonthV813(month){
  return (state.budgets||[]).filter(b=>b&&b.month===month);
}
function ensureRecurringBudgetsV813(month,depth=0){
  const target=String(month||'');
  if(!/^\d{4}-\d{2}$/.test(target)||depth>24)return 0;
  const current=monthKey();
  if(target<current)return 0;

  const previous=shiftMonth(target,-1);
  if(previous>=current && !budgetsForMonthV813(previous).length){
    ensureRecurringBudgetsV813(previous,depth+1);
  }

  const source=budgetsForMonthV813(previous).filter(b=>b.carryForward!==false);
  ...
  source.forEach(b=>{
    ...
    if(!category)return;
    const exists=(state.budgets||[]).some(x=>x&&x.month===target&&x.category===category);
    if(exists||isBudgetSkippedV813(target,category))return;
    state.budgets.push({
      id:uid(),month:target,category,limit:Number(b.limit)||0,
      carryForward:b.carryForward!==false,
      inheritedFrom:String(b.id||''),inheritedFromMonth:previous
    });
    added++;
  });
...
function copyPreviousMonthBudgetsV813(month){
  const target=String(month||planPeriod);
  const previous=shiftMonth(target,-1);
  const source=budgetsForMonthV813(previous);
  if(!source.length){toast(`Belum ada budget pada ${monthLabel(previous)}.`);return;}
  ...
  source.forEach(b=>{
    ...
    const exists=(state.budgets||[]).some(x=>x&&x.month===target&&x.category===category);
    if(exists)return;
    setBudgetSkipV813(target,category,false);
    state.budgets.push({
      id:uid(),month:target,category,limit:Number(b.limit)||0,
      carryForward:b.carryForward!==false,
      inheritedFrom:String(b.id||''),inheritedFromMonth:previous
  ...
  if(!copied){toast('Semua kategori budget bulan lalu sudah ada di bulan ini.');return;}
  save();toast(`${copied} budget dari ${monthLabel(previous)} berhasil disalin.`);scheduleRenderV812();
}
function requestDeleteBudgetV813(id){
  const budget=(state.budgets||[]).find(v=>v&&v.id===id);
  if(!budget)return;
  openConfirm('Hapus Budget',`Hapus budget ${budget.category} untuk ${monthLabel(budget.month)}? Budget bulan lain tidak ikut terhapus.`,'Hapus',()=>{
    setBudgetSkipV813(budget.month,budget.category,true);
    state.budgets=state.budgets.filter(v=>v.id!==budget.id);
    save();toast('Budget bulan ini dihapus.');scheduleRenderV812();
  });
}

function openBudgetModal(editId=''){
 const b=editId?state.budgets.find(x=>x.id===editId):null;
 const carryForward=b?.carryForward===undefined?true:!!b.carryForward;
 openModal(b?'Edit Budget':'Atur Budget',`<form class="form">
   ...
   <label class="budget-repeat-row-v813">
     <span><b>Gunakan juga untuk bulan berikutnya</b><small>Nominal otomatis menjadi template bulan berikutnya. Pemakaian bulan ini tidak ikut terbawa.</small></span>
     <span class="switch"><input type="checkbox" name="carryForward" ${carryForward?'checked':''}><span></span></span>
   </label>
   ...
   const carryForward=fd.get('carryForward')==='on';
   ...
   if(b){
     const oldMonth=b.month,oldCategory=b.category;
     Object.assign(b,{month,category,limit,carryForward});
     if(oldMonth!==month||oldCategory!==category)setBudgetSkipV813(oldMonth,oldCategory,true);
   }else{
     state.budgets.push({id:uid(),month,category,limit,carryForward,inheritedFrom:'',inheritedFromMonth:''});
   }
   setBudgetSkipV813(month,category,false);
   planPeriod=month;localStorage.setItem('uangku_plan_period',planPeriod);
   save();closeModal();toast(b?'Budget diperbarui.':'Budget ditambahkan.');scheduleRenderV812();
 });
}
...
function openDebtModal(editId=''){
 ...
 openModal(d?'Edit Utang / Piutang':'Tambah Utang / Piutang',`<form class="form">
   ...
   save();closeModal();toast(d?'Catatan diperbarui.':'Catatan ditambahkan.');scheduleRenderV812();
 });
}
...
function markDebtPaid(id){
 ...
 openConfirm('Tandai Lunas',`Tandai ${d.type==='payable'?'utang':'piutang'} ${d.person} sebagai lunas?`,'Tandai Lunas',()=>{
   d.paid=debtTotalAmount(d);d.status='paid';d.paidDate=today();save();toast('Ditandai lunas.');scheduleRenderV812();
 });
}
...
function requestDeleteDebt(id){
 ...
 openConfirm('Hapus Catatan',`Hapus catatan ${d.person}?`,'Hapus',()=>{
   state.debts=state.debts.filter(x=>x.id!==id);save();toast('Catatan dihapus.');scheduleRenderV812();
 });
}
...
function openBillModal(editId=''){
 ...
 openModal(b?'Edit Tagihan':'Tambah Tagihan',`<form class="form">
   ...
   save();closeModal();toast(b?'Tagihan diperbarui.':'Tagihan ditambahkan.');scheduleRenderV812();
 });
}
...
function requestDeleteBill(id){
 ...
 openConfirm('Hapus Tagihan',`Hapus tagihan ${b.name}?`,'Hapus',()=>{
   ...
   save();toast('Tagihan dihapus.');scheduleRenderV812();
 });
}
...
function bindPage(){
 ...
 const bindPeriod=(id,get,set)=>{
   ...
   if(input)input.onchange=e=>{
     if(/^\d{4}-\d{2}$/.test(e.target.value)){set(e.target.value);scheduleRenderV812()}
   };
   $$(`[data-period-prev="${id}"]`).forEach(btn=>btn.onclick=()=>{set(shiftMonth(get(),-1));scheduleRenderV812()});
   $$(`[data-period-next="${id}"]`).forEach(btn=>btn.onclick=()=>{set(shiftMonth(get(),1));scheduleRenderV812()});
 };

 if(currentPage==='home'){
   $('#toggleBalanceBtn')&&($('#toggleBalanceBtn').onclick=toggleBalanceVisibility);
   $('#toggleNetWorthDetail')&&($('#toggleNetWorthDetail').onclick=()=>{netWorthExpanded=!netWorthExpanded;localStorage.setItem('uangku_networth_expanded',netWorthExpanded?'1':'0');scheduleRenderV812()});
   $('#includeAssetsToggle')&&($('#includeAssetsToggle').onchange=e=>{includeInvestmentAssets=!!e.target.checked;localStorage.setItem('uangku_include_investment_assets',includeInvestmentAssets?'1':'0');scheduleRenderV812()});
   ...
   $$('[data-plan-tab]').forEach(btn=>btn.onclick=()=>{planTab=btn.dataset.planTab==='goal'?'goal':'budget';scheduleRenderV812()});
   $('#addBudgetBtn')&&($('#addBudgetBtn').onclick=()=>openBudgetModal());
   $('#copyPreviousBudgetV813')&&($('#copyPreviousBudgetV813').onclick=()=>copyPreviousMonthBudgetsV813(planPeriod));
   $$('[data-edit-budget]').forEach(btn=>btn.onclick=()=>openBudgetModal(btn.dataset.editBudget));
   $$('[data-delete-budget]').forEach(btn=>btn.onclick=()=>requestDeleteBudgetV813(btn.dataset.deleteBudget));
   ...
   $$('[data-asset-tab]').forEach(btn=>btn.onclick=()=>{assetTab=['investment','property','physical'].includes(btn.dataset.assetTab)?btn.dataset.assetTab:'investment';scheduleRenderV812()});
   ...
   $$('[data-debt-tab]').forEach(btn=>btn.onclick=()=>{const next=btn.dataset.debtTab;debtTab=['debts','bills','routines'].includes(next)?next:'debts';scheduleRenderV812()});
...
function totalBalance(){
 return financeTotalsV812().balance;
}

function savingsBalance(){
 return financeTotalsV812().savings;
}
...
function financeTotalsV812(){
 const cache=ensureDerivedCacheV83();
 if(cache.financeTotals)return cache.financeTotals;
 ...
 (state.accounts||[]).forEach(a=>{
   ...
   balance+=value;
   if(String(a.type||'').toLowerCase()==='tabungan')savings+=value;
 });
 ...
 (state.debts||[]).forEach(d=>{
   ...
   if(d.type==='receivable')receivable+=left;
   else payable+=left;
 });

 let assets=0;
 ['investment','property','physical'].forEach(tab=>{
   (state.assets?.[tab]||[]).forEach(a=>{
     assets+=assetCurrentValue(a,tab);
   });
 });

 let goalAllocated=0;
 (state.goals||[]).forEach(g=>{
   goalAllocated+=goalLinkedTotal(g);
 });

 cache.financeTotals={balance,savings,payable,receivable,assets,goalAllocated};
 return cache.financeTotals;
}

function totalAssets(){
 return financeTotalsV812().assets;
}

function budgetStatsV812(k=homePeriod){
 const cache=ensureDerivedCacheV83();
 if(cache.budgetStats.has(k))return cache.budgetStats.get(k);
 ...
 categories.forEach(cat=>{
   used+=Number(byCategory.get(cat)||0);
 });
 budgets.forEach(b=>{
   remaining+=Math.max(
     0,
     (Number(b.limit)||0)-(Number(byCategory.get(b.category))||0)
   );
 });

 const livingTotal=monthlyMoneyAllocation(k).living;
 const result={
   budgets,
   total,
   used,
   ...
   outsideBudget:Math.max(0,livingTotal-used),
   byCategory
 };

 cache.budgetStats.set(k,result);
 return result;
}

function budgetRemainingTotal(k=homePeriod){
 return budgetStatsV812(k).remaining;
}

function netWorthValue(){
 const x=financeTotalsV812();
 return x.balance+x.receivable+(includeInvestmentAssets?x.assets:0)-x.payable;
}

function setTxType(t){
 manualTxType=['income','expense','transfer'].includes(t)?t:'income';
 if(currentPage==='addTransaction')scheduleRenderV812();
}
...
function openAssetModal(editId=''){
 ...
 if(assetTab!=='physical'){
   openModal(a?`Edit ${tabLabel}`:`Tambah ${tabLabel}`,`<form class="form">
     ...
     <div class="field"><label>Logo / Gambar</label>${assetLogoEditorHTMLV8131(assetTab,a)}</div>
 ...
 openModal(a?'Edit Aset Fisik':'Tambah Aset Fisik',`<form class="form">
   ...
   <div class="field"><label>Logo / Gambar</label>${assetLogoEditorHTMLV8131('physical',a)}</div>
 ...
 setTimeout(()=>{
   ...
   bindAssetLogoEditorV8131('physical',a);
 },0);
}
...
function totalGoalAllocated(){
 return financeTotalsV812().goalAllocated;
}
...
function openGoalModalCore(editId=''){
 ...
 openModal(g?'Edit Target':'Tambah Target',`<form class="form">
   ...
   save();closeModal();toast(g?'Target diperbarui.':'Target ditambahkan.');scheduleRenderV812();
 });
...
function normalizeState(raw){
 ...
 return {
   ...
   budgets:Array.isArray(x.budgets)?x.budgets.filter(Boolean).map(b=>({
     ...b,
     id:b.id||uid(),
     month:String(b.month||''),
     category:String(b.category||''),
     limit:Number(b.limit)||0,
     carryForward:b.carryForward===undefined?true:!!b.carryForward,
     inheritedFrom:String(b.inheritedFrom||''),
     inheritedFromMonth:String(b.inheritedFromMonth||'')
   })):[],
   budgetSkips:x.budgetSkips&&typeof x.budgetSkips==='object'?x.budgetSkips:{},
   assets:{
...
function transactionDayGroupV812(date,rows){
 const safeDate=esc(date);
 return `<div class="tx-day-group-v812" data-tx-date="${safeDate}">
   <div class="day-title">${prettyDate(date)}</div>
   <div class="list transaction-list-v56">${rows.map(txItem).join('')}</div>
 </div>`;
}

function transactionFooterHTMLV812(total,shown){
 const remaining=Math.max(0,total-shown);
 if(remaining>0){
   return `<div class="transaction-load-more-wrap-v811">
     ...
     <small>${shown} dari ${total} transaksi ditampilkan</small>
   </div>`;
 }
 return total>TX_PAGE_SIZE_V811
   ?`<div class="transaction-load-more-wrap-v811 done"><small>Semua ${total} transaksi sudah ditampilkan.</small></div>`
   :'';
}

function groupTransactionRowsV812(rows){
 const groups=[];
 rows.forEach(t=>{
...
function renderTransactionResultsV811(){
 ...
 const shown=Math.min(txVisibleCountV811,tx.length);
 const visible=tx.slice(0,shown);
 const groups=groupTransactionRowsV812(visible);

 return `<div id="transactionListV812">
   ${groups.length
     ?groups.map(g=>transactionDayGroupV812(g.date,g.rows)).join('')
     :'<div class="empty" id="transactionEmptyV812">Belum ada transaksi.</div>'}
 </div>
 <div id="transactionFooterV812">${transactionFooterHTMLV812(tx.length,shown)}</div>`;
}
...
function appendTransactionsV812(){
 const tx=filteredTransactionsV811(txFilter);
 const start=Math.min(txVisibleCountV811,tx.length);
 const end=Math.min(start+TX_PAGE_SIZE_V811,tx.length);
 if(end<=start)return;

 const list=$('#transactionListV812');
 if(!list){
   txVisibleCountV811=end;
   refreshTransactionResultsV811();
   return;
 }

 $('#transactionEmptyV812')?.remove?.();

 const page=tx.slice(start,end);
 const groups=groupTransactionRowsV812(page);

 groups.forEach(group=>{
   const existing=list.querySelector?.(`[data-tx-date="${group.date}"]`);
   if(existing){
     const rowsHost=existing.querySelector?.('.transaction-list-v56');
     if(rowsHost)rowsHost.insertAdjacentHTML('beforeend',group.rows.map(txItem).join(''));
   }else{
     list.insertAdjacentHTML('beforeend',transactionDayGroupV812(group.date,group.rows));
   }
 });

 txVisibleCountV811=end;

 const footer=$('#transactionFooterV812');
 if(footer)footer.innerHTML=transactionFooterHTMLV812(tx.length,end);
}
...
function openEditTransactionModal(id){
 ...
 if(t.type==='transfer'){
   openModal('Edit Transaksi',`<form class="form">
     ...
     save();closeModal();toast('Transaksi diperbarui.');scheduleRenderV812();
   });
   return;
 }
...
function requestDeleteTransactionCore(id){
 ...
 openConfirm('Hapus Transaksi',`Hapus transaksi ${t.note||t.category||'ini'}?${extra}`,'Hapus',()=>{
   ...
   save();toast('Transaksi dihapus.');scheduleRenderV812();
 });
}
...
function budgetedLivingUsedForMonth(k){
 return budgetStatsV812(k).used;
}

function renderPlans(){
 ensureRecurringBudgetsV813(planPeriod);
 const stats=budgetStatsV812(planPeriod);
 const budgets=stats.budgets;
 const alloc=budgetAllocation(budgets);
 const totalBudget=stats.total;
 const totalUsed=stats.used;
 const totalUsedPct=stats.pct;
 const totalRemaining=Math.max(0,totalBudget-totalUsed);
 const outsideBudget=stats.outsideBudget;
 ...
 ${planTab==='budget'?`
   ...
   <div class="budget-subsection-title budget-subsection-title-v813">
     <div><h3>Rincian Budget Hidup</h3><p>Batas pengeluaran harian dan kebutuhan per kategori</p></div>
     <button type="button" class="mini-action budget-copy-prev-v813" id="copyPreviousBudgetV813">Salin bulan lalu</button>
   </div>
 ...
 return `<section class="card home-budget-card">
   ...
   ${budgets.length?`
     <div class="home-budget-main">
       <div class="donut-holder home-budget-donut home-donut-standard-v8121">
         <div class="donut" style="background:${budgetGradient(budgets)}"></div>
...
function monthlyHomeSummaryV65(k){
  ...
  const budgetStats=budgetStatsV812(k);
  const totalBudget=budgetStats.total;
  const budgetUsed=budgetStats.used;
...
function debtReceivableGradientV70(){
  const d=debtTotals();
  const payable=Math.max(0,Number(d.payable)||0);
  const receivable=Math.max(0,Number(d.receivable)||0);
  const total=payable+receivable;

  if(total<=0)return '#E9EFE7';

  const payablePct=Math.max(0,Math.min(100,payable/total*100));
  const cut=Number(payablePct.toFixed(4));

  return `conic-gradient(
    ${debtDonutColorsV8121.payable} 0% ${cut}%,
    ${debtDonutColorsV8121.receivable} ${cut}% 100%
  )`;
}
...
function renderDebtReceivableHomeV70(){
  ...
  const payablePct=total>0?d.payable/total*100:0;
  const receivablePct=total>0?d.receivable/total*100:0;
    ...
    ${total>0?`
      <div class="home-debt-main-v70">
        ...
        <div class="home-debt-stats-v70">
          <div>
            <span><i class="debt-dot-v70 payable"></i>Sisa Utang <small>${payablePct.toFixed(1)}%</small></span>
            <b style="color:${debtDonutColorsV8121.payable}">${fmt(d.payable)}</b>
          </div>
          <div>
            <span><i class="debt-dot-v70 receivable"></i>Sisa Piutang <small>${receivablePct.toFixed(1)}%</small></span>
            <b style="color:${debtDonutColorsV8121.receivable}">${fmt(d.receivable)}</b>
          </div>
        </div>
      </div>
    `:`
  ...
  return `<section class="card home-sales-card">
    ...
    ${has?`
      <div class="home-sales-main">
        <div class="donut-holder home-sales-donut home-donut-standard-v8121">
...
function budgetCard(b,i){
  ...
  >
    ...
    <div class="budget-info">
      <div class="goal-row">
        <div><div class="list-title">${esc(b?.category||'Tanpa kategori')}</div>${b?.carryForward!==false?'<span class="budget-repeat-chip-v813">↻ lanjut bulan depan</span>':''}</div>
...
function renderHome(){
  ensureRecurringBudgetsV813(homePeriod);
